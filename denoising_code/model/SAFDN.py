...
from denoising_code.model.stdc_wb import STDCNet, ConvX
...
class UAFM(nn.Module):      # 改进的融合注意力机制(加入空间注意力模块)
    """Unified Attention Fusion Modul
    """

    def __init__(self, low_chan, hight_chan, out_chan) -> None:
        """
        ...
        self.conv_atten_s = nn.Sequential(
            ConvX(4, 2, kernel_size=3),
            nn.Conv2d(2, 1, kernel_size=3, padding=1, bias=False),
            nn.BatchNorm2d(1),
        )

        self.conv_atten_c = nn.Sequential(
    ...
    def forward(self, x_high, x_low):
        ...
        atten = torch.sigmoid(self.conv_atten_c(atten))
        ...
        atten = torch.concat([atten_high, atten_low], dim=1)
        atten = torch.sigmoid(self.conv_atten_s(atten))
...
if __name__ == '__main__':
    ...
    model.load_state_dict(
        torch.load('/home/tony/github/SAFDN/checkpoints_PPliteseg_both/model_epoch43_mIoU=90.4.pth'))
    model.eval()
    model.to('cuda')
