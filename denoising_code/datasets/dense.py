...
class DENSE(data.Dataset):
    ...
    def __init__(self, root, split='train', transform=None):
        ...
        self.split_full = os.path.join(self.root, '{}_01'.format(split))
        ...
        # 读取‘{}_01'中的各个子文件中的文件名
        self.lidar = [os.path.join(r,file) for r,d,f in os.walk(self.split_full) for file in f]
        # print(self.split_full)
        # print(len(self.lidar))
        if split == 'train':
            other_split = os.path.join(self.root, '{}_02'.format(split))
            # print(other_split)
            self.lidar += [os.path.join(r,file) for r,d,f in os.walk(other_split) for file in f]
    ...
    def __getitem__(self, index):
        with h5py.File(self.lidar[index], "r", driver='core') as hdf5:
            ...
            label_1 = np.vectorize(label_dict.get)(label_1) # 根据字典的key-value 替换每个label值
        ...
        if self.transform:
            ...
            #输出预测点云图 使用getlable.py保存为h5df文件时需要打开下面的注释
...
if __name__ == '__main__':
    ...
    dataset = DENSE('../../cnn_denoising',split= 'train', transform=joint_transforms)
